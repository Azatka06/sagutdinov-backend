...
import sagutdinov.exception.*
import sagutdinov.service.*
import sagutdinov.repository.PostRepository
import sagutdinov.repository.PostRepositoryInMemoryWithMutexImpl
import sagutdinov.repository.UserRepository
import sagutdinov.repository.UserRepositoryInMemoryWithAtomicImpl
...
fun Application.module(testing: Boolean = false) {
    ...
    install(KodeinFeature) {
        ...
        constant(tag = "fcm-password") with (environment.config.propertyOrNull("sagutdinov.fcm.password")?.getString()
                ?: throw ConfigurationException("FCM Password is not specified"))
        constant(tag = "fcm-salt") with (environment.config.propertyOrNull("sagutdinov.fcm.salt")?.getString()
                ?: throw ConfigurationException("FCM Salt is not specified"))
        constant(tag = "fcm-db-url") with (environment.config.propertyOrNull("sagutdinov.fcm.db-url")?.getString()
                ?: throw ConfigurationException("FCM DB Url is not specified"))
        constant(tag = "fcm-path") with (environment.config.propertyOrNull("sagutdinov.fcm.path")?.getString()
